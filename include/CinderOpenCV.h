...
#ifndef VISUALS_CINDEROPENCV_H
#define VISUALS_CINDEROPENCV_H
...
namespace cinder {
    ...
    public:
        static std::shared_ptr<ImageTargetCvMat> createRef(cv::Mat *mat) {
            return std::shared_ptr<ImageTargetCvMat>(new ImageTargetCvMat(mat));
        }
...
// ImageTargetCvMat
    ...
    inline cv::Mat toOcvRef(Channel8u &channel) {
        return cv::Mat(channel.getHeight(), channel.getWidth(), CV_MAKETYPE(CV_8U, 1), channel.getData(),
                       channel.getRowBytes());
    }

    inline cv::Mat toOcvRef(Channel16u &channel) {
        return cv::Mat(channel.getHeight(), channel.getWidth(), CV_MAKETYPE(CV_16U, 1), channel.getData(),
                       channel.getRowBytes());
    }

    inline cv::Mat toOcvRef(Channel32f &channel) {
        return cv::Mat(channel.getHeight(), channel.getWidth(), CV_MAKETYPE(CV_32F, 1), channel.getData(),
                       channel.getRowBytes());
    }

    inline cv::Mat toOcvRef(Surface8u &surface) {
        return cv::Mat(surface.getHeight(), surface.getWidth(), CV_MAKETYPE(CV_8U, surface.hasAlpha() ? 4 : 3),
                       surface.getData(), surface.getRowBytes());
    }

    inline cv::Mat toOcvRef(Surface16u &surface) {
        return cv::Mat(surface.getHeight(), surface.getWidth(), CV_MAKETYPE(CV_16U, surface.hasAlpha() ? 4 : 3),
                       surface.getData(), surface.getRowBytes());
    }

    inline cv::Mat toOcvRef(Surface32f &surface) {
        return cv::Mat(surface.getHeight(), surface.getWidth(), CV_MAKETYPE(CV_32F, surface.hasAlpha() ? 4 : 3),
                       surface.getData(), surface.getRowBytes());
    }
